...
export default function Gad7DoneScreen() {
  const scores = useMindScreeningStore((s) => s.gad7Scores);
  const clearGad7 = useMindScreeningStore((s) => s.clearGad7);

  const { total, label } = useMemo(() => {
    const t =
      scores?.length === 7 ? scores.reduce((a, b) => a + b, 0) : null;
    return {
      total: t,
      label: t != null ? gad7SeverityLabel(t) : null,
    };
  }, [scores]);
      ...
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        >
          ...
          <View style={styles.xpCard}>
            ...
            <Text style={[styles.xpTxt, mindTextNoSelectWeb]}>+25 XP</Text>
          </View>
